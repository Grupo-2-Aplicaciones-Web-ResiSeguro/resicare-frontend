  ...
  >
    <header class="page-header">
      <pv-button
          ...
          :label="t('common.back')"
      />
      <h1 id="page-title">{{ t('iam.profile.editTitle') || 'Editar perfil' }}</h1>
    </header>
      ...
      >
        ...
        <div class="form-group">
          <label id="password-label" for="password" class="sr-only">
            {{ t('iam.profile.passwordPlaceholder') }}
          </label>
          <pv-input-text
              id="password"
              v-model="password"
              type="password"
              :placeholder="t('iam.profile.passwordPlaceholder')"
              :minlength="8"
        ...
        <div class="form-group">
          <label id="bio-label" for="bio" class="sr-only">
            {{ t('iam.profile.placeholderBio') }}
          </label>
          <pv-textarea
              id="bio"
              v-model="bio"
              :placeholder="t('iam.profile.placeholderBio')"
              rows="4"
              :maxlength="500"
              aria-labelledby="bio-label"
              aria-describedby="bio-hint"
          />
          <span id="bio-hint" class="hint-text">
            {{ bio.length }}/500
          </span>
        </div>
...
onMounted(async () => {
  ...
  try {
    ...
    // Normalize values for form fields (handle object/string formats)
...
/**
 * Submits profile updates with backend validation
 */
async function onSubmit() {
  ...
  try {
    ...
    if (e.response && e.response.data) {
      globalError.value = e.response.data.message || t('iam.profile.saveError')
    } else {
